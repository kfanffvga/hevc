...
void ProfileTierLevel::ParseSubLayerInfo(BitStream* bit_stream)
{
    ...
    --max_num_sub_layers_;
    auto sub_layers_range = boost::extents[max_num_sub_layers_];
    ...
    if (max_num_sub_layers_ > 0)
        bit_stream->SkipBits(max(8 - max_num_sub_layers_, 0) << 1);
    ...
    multi_array<bool, 2> has_sub_layer_profile_compatibility(
        boost::extents[max_num_sub_layers_][compatibility_flag_count]);
    ...
    for (int i = 0; i < max_num_sub_layers_; ++i)
    {
        if (has_sub_layer_profile_present[i])
        {
            ...
            is_sub_layer_tier[bit_stream->ReadBool()];
            ...
            else
            {
                ...
                {
                    // reserved zero
                    assert(bit_stream->ReadBool() == false);
                }
            }
        }
