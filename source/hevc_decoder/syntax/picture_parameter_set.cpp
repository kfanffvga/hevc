...
bool PictureParameterSet::Parse(BitStream* bit_stream)
{
    ...
    uint8 num_extra_slice_header_bits = bit_stream->Read<uint8>(3);
    bool is_sign_data_hiding_enabled = bit_stream->ReadBool();
    bool has_cabac_init_present = bit_stream->ReadBool();
    uint32 num_ref_idx_l0_default_active = golomb_reader.ReadUnsignedValue() + 1;
    uint32 num_ref_idx_l1_default_active = golomb_reader.ReadUnsignedValue() + 1;
    ...
    if (is_cu_qp_delta_enabled)
        uint32 diff_cu_qp_delta_depth = golomb_reader.ReadUnsignedValue();
    ...
    bool has_weighted_bipred_flag = bit_stream->ReadBool();
